...
export type FilterBarProps = {
  ...
  fieldOrder?: Array<"query" | "date" | "status">;
...
export function FilterBar({
  ...
  const fields = fieldOrder.filter((f, i, arr) => arr.indexOf(f) === i);
    ...
    <div className={`flex flex-col gap-4 mb-3 ${className ?? ""}`}>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 w-full">
        {fields.map((field) => {
          if (field === "query") {
            return (
              <div key="query" className="flex flex-col w-full">
                <Label htmlFor="filter-q" className="mb-2 text-sm font-medium">
                  {labels?.query ?? "Cari Nama"}
                </Label>
          ...
          if (field === "date") {
            return (
              <div key="date" className="flex flex-col w-full">
                <Label
                  htmlFor="filter-date"
                  className="mb-2 text-sm font-medium px-0"
                >
                  {labels?.date ?? "Tanggal"}
                </Label>
              ...
              <div key="status" className="flex flex-col w-full">
                ...
                >
                  <SelectTrigger id="filter-status" className="w-full h-9">
                    <SelectValue
                      placeholder={placeholders?.status ?? "Pilih status"}
                    />
                  </SelectTrigger>
                  <SelectContent>
