...
export function ProfilesInstantSearchForm() {
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
      <div
        className="flex flex-col gap-2 flex-1 min-w-[260px] relative"
        id="profiles-search-wrapper"
      >
        <label htmlFor="profiles-name" className="text-sm font-medium">
          Cari Nama
        </label>
        <Input
          id="profiles-name"
          ...
          autoComplete="off"
          onFocus={() => {
            if (enableSuggest) setOpenSuggest(true);
          }}
          onChange={(e) => {
          ...
          <div className="absolute z-20 top-full mt-1 w-full rounded-md border bg-popover shadow-md max-h-72 overflow-auto">
            <div className="p-1 text-xs text-muted-foreground flex items-center justify-between">
              <span>
                {suggestLoading
                  ? "Memuat..."
                  : suggestData?.data?.length
                    ? `${suggestData.data.length} hasil`
                    : "Tidak ada hasil"}
              </span>
